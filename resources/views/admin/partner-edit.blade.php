    ...
    <form action="{{ Request::url() }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-9 mb-2">
                <input type="text" name="title" required="required" class="form-control" value="{{ $partner->title }}" maxlength="255">
            </div>
        </div>
        <div class="row">
            <div class="col-12 col-lg-3">
            ...
            <div class="col-12 col-lg-9 mb-2">
                <div class="input-group">
                    <span class="input-group-text" id="basic-addon3">{{ url('/') }}/partner/</span>
                    <input type="text" name="slug" value="{{ $partner->slug }}" required="required" class="form-control">
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-12 col-lg-3">
                Featured Image URL <i class="fas fa-info-circle"  data-bs-toggle="tooltip" data-bs-placement="right" title="Recommended Resolution (1300 x 450 pixels). Image will be cropped from center in the partner list page, but will show full size in the single partner page."></i>
            </div>
            <div class="col-12 col-lg-9 mb-2">
                <input type="text" name="featured_image" value="{{ $partner->featured_image }}" placeholder="Image URL (leave empty if no featured image)" class="form-control">
        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-9 mb-2">
                <select name="is_pinned" class="form-select">
                    <option value="0" {{ ($partner->is_pinned == 0) ? 'selected="selected"' : ''; }}>No</option>
                    <option value="1" {{ ($partner->is_pinned == 1) ? 'selected="selected"' : ''; }}>Yes</option>
                </select>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-12 col-lg-9 mb-2">
                <select name="language" required="required" class="form-select">
                    ...
                    <option value="ur" {{ ($language == 'ur') ? 'selected="selected"' : ''; }}>Urdu / Hindi</option>
                </select>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-12 col-lg-12 mb-2">
                <textarea name="news_content" rows="25" class="form-control">{{ $partner->news_content }}</textarea>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
...
<script>
    ...
    $('input[name=language]').on('change click', function(){
        ...
        console.log(lang);
        if (lang != 'en') {
            $('#basic-addon3').text('{{ url('/') }}/'+lang+'/partner/');
        } else {
            $('#basic-addon3').text('{{ url('/') }}/partner/');
        }
    });
